...
app.use(cors({ origin: ['https://link-in-bio-2.web.app', 'http://localhost:5173'] }));
...
app.post("/links/:linkId/click", async (req, res) => {
  try {
    ...
    const { ownerUsername } = req.body; // Username van de pagina eigenaar

    if (!linkId || !ownerUsername) {
        return res.status(400).send({ message: "linkId and ownerUsername are required." });
    }
    ...
    const linkClicksRef = db.ref(`analytics/${ownerUsername}/clicks/${linkId}/count`);
     ...
     const clickTimestampRef = db.ref(`analytics/${ownerUsername}/clicks/${linkId}/timestamps`);
    ...
    console.error("Error in /links/:linkId/click POST:", error);
